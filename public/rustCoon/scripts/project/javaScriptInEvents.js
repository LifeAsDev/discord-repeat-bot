...
const scriptsInEvents = {
	...
	async EventWorld_Event19(runtime, localVars) {
		...
		const terrain = generateTerrain(
			runtime.globalVars.tileWidth,
			runtime.globalVars.tileHeight,
			tiles
		);
	...
	async EventWorld_Event444(runtime, localVars) {
		const jsonString = {
			px: localVars.px,
			py: localVars.py,
			animationName: localVars.animationName,
			mirror: localVars.mirror,
			inputs: localVars.inputs,
			id: localVars.id,
		};
	...
	async EventWorld_Event445(runtime, localVars) {
		localVars.jsonStringify = JSON.stringify({
			playersArr: runtime.playersArr,
			date: localVars.date,
		});
	},
	...
	async EventWorld_Event474_Act1(runtime, localVars) {
		...
		for (let i = 0; i < data.playersArr.length; i++) {
			runtime.callFunction(
				"playerSync",
				data.date,
				data.playersArr[i].animationName,
				data.playersArr[i].mirror,
				data.playersArr[i].px,
				data.playersArr[i].py,
				data.playersArr[i].inputs,
				data.playersArr[i].id
			);
		}
	},
