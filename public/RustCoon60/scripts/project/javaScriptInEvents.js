...
async function saveRoomData(nombre, data) {
  ...
  const res = await fetch(`${API_BASE}/storage/save`, {
    method: "PATCH",
...
async function loadRoomData(nombre) {
  try {
    const res = await fetch(
      `${API_BASE}/storage/load/${encodeURIComponent(nombre)}`
    );
...
function processQueue(runtime) {
  if (isLoadingRoom || roomQueue.length === 0) return;
...
const scriptsInEvents = {
	...
	async EventConnect_Event4(runtime, localVars)
	{
...
fetch(`${API_BASE}/rooms`)
  ...
  .catch(err => console.error(err));
	...
	async EventWorld_Event16_Act1(runtime, localVars)
	{
	...
	async EventWorld_Event18_Act17(runtime, localVars)
	{
		const roomName = localVars.room;
		const data = await loadRoomData(localVars.room);
		
		if (data === null) {
		runtime.callFunction("hostStartGame","",roomName)
		}
		 else {
		  runtime.callFunction("hostStartGame",data,roomName)
		}
		
	},

	async EventWorld_Event29_Act1(runtime, localVars)
	{
		    isLoadingRoom = false;
		    processQueue(runtime);
	},

	async EventWorld_Event35(runtime, localVars)
	{
		/**
